...
class DatatablesService {
    ...
    /**
     * Get Container from config parameter
     */
    ...
    public function init($connectionType = 'default') {
        $this->entityManager = $this->container->get('doctrine')->getManager($connectionType);
...
//        $this->distinct = $column;
        $this->queryBuilder->distinct($column); //Changed by Yashvant zala on fetch distinct error time add this line and comment other line
//        $this->ci->db->distinct($column);
     ...
     * @return mixed
     */
    public function join($entity, $alias, $type = null, $fk = null) {
        if ($type != null && $fk != null)
            $this->queryBuilder->innerJoin($entity, $alias, $type, $fk);
        else
            $this->queryBuilder->innerJoin($entity, $alias);
        return $this;
    }
     ...
     * @return mixed
     */
    public function leftjoin($entity, $alias, $type = null, $fk = null) {
        if ($type != null && $fk != null)
            $this->queryBuilder->leftJoin($entity, $alias, $type, $fk);
        else
            $this->queryBuilder->leftJoin($entity, $alias);
        return $this;
    }
    ...
    public function like($key, $val) {
        $this->queryBuilder->where($this->queryBuilder->expr()->like($key, $this->queryBuilder->expr()->literal('%' . $val . '%')));
        return $this;
    }
    ...
    public function orlike($key, $val) {
        $this->queryBuilder->orwhere($this->queryBuilder->expr()->like($key, $this->queryBuilder->expr()->literal('%' . $val . '%')));
        return $this;
    }
            ...
            for ($i = 0; $i < intval($_REQUEST['iSortingCols']); $i++)
                if (isset($mColArray[intval($_REQUEST['iSortCol_' . $i])]) && in_array($mColArray[intval($_REQUEST['iSortCol_' . $i])], $columns) && $_REQUEST['bSortable_' . intval($_REQUEST['iSortCol_' . $i])] == 'true')
                    $this->queryBuilder->add(
                            'orderBy', $mColArray[intval($_REQUEST['iSortCol_' . $i])] . ' ' . $_REQUEST['sSortDir_' . $i]);
    ...
    private function get_filtering() {
        ...
        if ($sSearch != '') {
            ...
            for ($i = 0; $i < $cnt; $i++) {
                if (isset($_REQUEST['bSearchable_' . $i]) && $_REQUEST['bSearchable_' . $i] == 'true' && in_array($mColArray[$i], $columns)) {
                    if (preg_match('[COUNT|MAX|MIN|AVG|SUM|DISTINCT|GROUPCONCAT]', strtoupper($this->select[$mColArray[$i]]))) {
                        // If any group function found then SKIP it.
                        $searchColumns--;
                        continue;
                    }
                    if ($orCondition == false) {
                        $conditionStr = '( ' . $this->queryBuilder->expr()->lower($this->select[$mColArray[$i]])
                                . ' like LOWER(' . $this->queryBuilder->expr()->literal('%' . $sSearch . '%') . ')';

                        $orCondition = true;
                        $searchColumns--;
                    } else if ($searchColumns == 1) {
                        $conditionStr .= ' OR ' . $this->queryBuilder->expr()->lower($this->select[$mColArray[$i]])
                                . ' like LOWER(' . $this->queryBuilder->expr()->literal('%' . $sSearch . '%') . ')';
                        // $conditionStr .= ' )'; //Chnage by Yashvant zala for IPbanned list error time 
                    } else {
                        $conditionStr .= ' OR ' . $this->queryBuilder->expr()->lower($this->select[$mColArray[$i]])
                                . ' like LOWER(' . $this->queryBuilder->expr()->literal('%' . $sSearch . '%') . ')';
                        $searchColumns--;
            ...
            $this->queryBuilder->andwhere($conditionStr .= ' )'); //Chang by Yashvant zala IPbanned list error time   ($this->queryBuilder->andwhere($conditionStr))
        } // End Search Filter    
                ...
                if (isset($_REQUEST['sSearch_' . $i]) && $_REQUEST['sSearch_' . $i] != '' && in_array($mColArray[$i], $columns)) {
                    ...
                    foreach ($miSearch as $val) {
                        if (preg_match("/(<=|>=|=|<|>)(\s*)(.+)/i", trim($val), $matches))
                            ...
                            $this->queryBuilder->andwhere(
                                    $this->select[$mColArray[$i]]
                                    . ' LIKE LOWER(' . $this->queryBuilder->expr()->literal('%' . $val . '%') . ') '
                            );
                    }
                }
            }
...
//        exit;
    ...
    private function get_total_results($filtering = FALSE) {
        ...
        $this->queryBuilder->resetDQLParts(array('groupBy', 'orderBy'));
